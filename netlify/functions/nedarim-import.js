...
function getAmount(tx) {
  return cleanNumber(
    tx.KevaAmount ?? tx.Amount ?? tx.amount ?? tx.Sum ?? tx.Total ?? tx.Schum ??
    tx.SchumHiyuv ?? tx.MonthlyAmount ?? tx.NextTashloum ?? tx.Tashloum ?? tx.HoraahAmount
  );
}
...
const KEVA_ROW_HINT_KEYS = [
  "KevaName", "KevaPhone", "KevaMail", "KevaAmount", "KevaStatus", "CreatedDate",
  "KevaNextDate", "KevaLastNum", "KevaGroupe", "KevaAvour",
  "ClientName", "Name", "FirstName", "Phone", "Mail", "Amount", "SchumHiyuv",
  "StartFrom", "NextDate", "LastNum", "PaymentType", "Groupe", "Category"
];
...
function parseResponse(raw, mode = "transaction") {
  ...
  const rows = collected.filter((row, index) => {
    ...
    const key = mode === "keva"
      ? String(row.KevaId || row.KevaID || row.HoraatKevaId || row.HoraaId || row.StandingOrderId || row.RecurringId || row.Id || row.id || `${row.KevaName || row.ClientName || row.Name || ""}|${row.KevaPhone || row.Phone || ""}|${row.KevaAmount || row.Amount || row.SchumHiyuv || ""}|${index}`)
...
function isInactiveKeva(tx) {
  const officialStatus = String(tx && tx.KevaStatus != null ? tx.KevaStatus : "").trim();
  if (officialStatus === "1") return false;
  if (officialStatus === "2" || officialStatus === "3") return true;

  const raw = [
    tx && tx.Status, tx && tx.status, tx && tx.Active, tx && tx.IsActive,
    tx && tx.Cancelled, tx && tx.Canceled, tx && tx.Bitual,
    tx && tx.KevaObservation, tx && tx.Notes, tx && tx.Comments
  ].filter((v) => v != null).join(" ").toLowerCase();
  return /מבוטל|בוטל|נמחק|מוקפא|cancel|inactive|deleted|frozen|לא\s*פעיל|הופסק/.test(raw);
}
...
function mapKevaToDashboardDonor(tx, index, mosadId) {
  ...
  const amount = cleanNumber(tx.KevaAmount ?? tx.Amount ?? tx.amount ?? 0);
  const currencyCode = String(tx.KevaCurrency ?? tx.Currency ?? tx.currency ?? "1").trim();
  const currency = currencyCode === "2" || /dollar|usd|\$|דולר/i.test(currencyCode) ? "USD" : "ILS";
  const counts = {
    remaining: numericDigits(tx.KevaTashlumim) || getKevaTashlumim(tx),
    completed: numericDigits(tx.KevaSuccess) || numericDigits(tx.HistoryCount) || "",
    total: ""
  };
  const remainingN = Number(counts.remaining || 0);
  const completedN = Number(counts.completed || 0);
  if (remainingN || completedN) counts.total = String(remainingN + completedN);

  const category = tx.KevaGroupe || tx.Groupe || tx.Group || tx.Category || "נציבי דעת אהרן";
  const comments = tx.KevaAvour || tx.KevaObservation || tx.Comments || tx.Comment || tx.Notes || tx.Avour || "";
  const clientName = tx.KevaName || tx.ClientName || tx.Name || [tx.FirstName, tx.LastName].filter(Boolean).join(" ") || "תורם ללא שם";
  const startDate = parseDate(tx.CreatedDate || tx.CreationDate || tx.StartFrom || tx.StartDate || tx.CreatedAt || tx.Date || tx.OpenDate || tx.TransactionTime);
  ...
  const officialStatus = String(tx.KevaStatus == null ? "" : tx.KevaStatus).trim();
  ...
  return {
    ...
    address: tx.KevaAdresse || tx.Adresse || tx.Address || tx.Street || "",
    city: tx.KevaCity || tx.City || "",
    category,
    isExternalDonation: true,
    importedFromNedarim: true,
    isExternalNedarim: true,
    ...
    memoryContent: comments,
    deceasedName: "",
    status: inactive ? (officialStatus === "2" ? "frozen" : "unpaid") : "paid",
    amount,
    currency,
    paymentProcessor: "Nedarim Plus",
    processor: "Nedarim Plus",
    paymentStatus: inactive ? (officialStatus === "2" ? "frozen" : "inactive") : "approved",
    ...
    remainingInstallments: counts.remaining,
    remainingCharges: counts.remaining,
    KevaSuccess: tx.KevaSuccess == null ? "" : String(tx.KevaSuccess),
    completedPayments: counts.completed,
    completedInstallments: counts.completed,
    completedCharges: counts.completed,
    totalPayments: counts.total,
    ...
    monthlyAmount: amount,
    installmentAmount: amount,
    totalCommitment: counts.total ? amount * Number(counts.total) : amount,
    currentMonthAmount: amount,
    TotalHistoryAmount: cleanNumber(tx.TotalHistoryAmount),
    HistoryCount: cleanNumber(tx.HistoryCount),
    paymentDate: startDate || new Date().toISOString(),
    startDate: startDate || "",
    nextChargeDate,
    dayOfCharge,
    KevaFrequency: tx.KevaFrequency == null ? "" : String(tx.KevaFrequency),
    KevaLastNum: tx.KevaLastNum || tx.LastNum || "",
    importedAt: new Date().toISOString(),
    ...
    notes: [
      ...
      `מזהה הוראת קבע: ${key}`,
      counts.remaining !== "" ? `יתרת חיובים: ${counts.remaining}` : "",
      tx.KevaSuccess != null && String(tx.KevaSuccess) !== "" ? `חיובים שבוצעו: ${tx.KevaSuccess}` : "",
      tx.HistoryCount != null && String(tx.HistoryCount) !== "" ? `מספר ניסיונות חיוב: ${tx.HistoryCount}` : "",
...
function groupSummary(rows) {
  ...
  for (const tx of rows || []) {
    const group = String(tx.KevaGroupe || tx.Groupe || tx.Group || tx.Category || "(ללא קטגוריה)").trim();
